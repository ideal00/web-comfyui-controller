...
- 尺寸选择器：先点**图片比例**（1:1 / 2:3 / 3:4 / 9:16 / 1:2 / 9:21 / 3:2 / 4:3 / 16:9 / 2:1 / 21:9），再用**长边滑块**（或直接输入长边 / 宽高）自由调整：短边按严格比例计算并**向下**对齐到 8 的倍数（例如 3:2 长边 1664 → 1664×1104、长边 1408 → 1408×936），面板实时显示当前尺寸、MP 与预计显存负载（Anima / Krea 2 在 8GB 上口径更紧，开了二采还会显示二采后的预计分辨率）。现有尺寸档位变成该比例下的**推荐档位**快捷按钮（`≈` 表示像素只是近似比例，例如 1216×832 实际 1.46:1、768×1344 实际 4:7），**每个比例会记住上次用的尺寸**；点比例时优先落在该比例的**第一个严格档位**（3:2 → 1536×1024、16:9 → 1280×720、9:16 → 720×1280），没有严格档位才退回近似档位；「宽 × 高」是**自定义精确尺寸**，改它只是临时允许非标准比例，再拖一次「长边」就回到当前选定比例。自定义尺寸会作为一个「自定义」档位写回下拉框，`payload()` / 预检 / 排队 / 快照恢复读的仍是同一个尺寸值。
- 左侧「快捷工具」抽屉全部可折叠：我的功能、**透明背景角色 PNG**、**任务批处理控制**、中文描述转换、读取 AI 图片参数；折叠状态会记住（`localStorage easyPanelRailFoldV1`，默认全部折叠），点「读取 DeepSeek 回答 / 读图还原 / 手动修正透明图」会自动展开对应面板。任务批处理折叠行的右侧常驻「执行中 / 排队 / 失败」短标签（队列空闲时显示「空闲」）。透明背景角色 PNG 从生成设置页移到这里，切换创作标签页也不会消失；抽屉标题栏的「⚙ 界面」小菜单收纳低频全局设置（输入框字号），不再占一整块卡片。
- 预览栏首屏留给图片：宽高滑块、当前队列与采样标签收进「⚙ 预览设置」，图片区域紧跟标题栏并在预览栏可见高度里居中；清晰度检测与「生成清晰版」改排在图片下方。
- 顶部「⋯ 更多」：≤1340px 时不再整体隐藏「框宽 / 功能编辑 / 打开 ComfyUI」，统一收进 `⋯ 更多` 下拉（≥1341px 自动平铺成原来的样子）。
- 生成结果工作台：每张结果附带状态卡（模型 / 尺寸 / Seed / 采样 / 二采 / LoRA），可直接「继续编辑 / 只换 Seed / 重复生成 / 只改服装、场景、姿势、表情 / 继续二采 / 转整图重绘 / 复制参数」；二采补充词只作用于高清阶段，不污染首采提示词。
- 分区替换（只改一项的底层能力）：点「换服装 / 换场景 / 换姿势 / 换表情」先打开替换面板——显示当前分区内容、可选自己的提示词预设或自定义输入、可切换「替换 / 追加」，并预览「哪个分区变了、哪些保持」，确认后才生成新版本；后端按 `sectionEdit` 只改指定分区（其余分区、模型、LoRA、尺寸、二采全部不变），生成快照/作品库会记录操作与旧→新值。提示词分区现在包含 人物与角色 / 外貌 / **表情** / 服装与材质 / 姿势 / 构图与镜头 / 场景 / 光线 / 画风与上色 / 自然语言 / 其他补充。
- 二采只保留成品：高清模式**不再另存首采原图**，输出目录与作品库只有二采后的成图；作品库里每张作品的**列表缩略图与详情大图固定指向同一张成品**（索引库按“非首采的最后一张输出”选定代表图，列表 `primary_artifact_id`、详情 `preview`、手机端预览三处一致）。旧任务里已经存在的 `<前缀>_base_*.png` 仍会在作品库输出列表里标注「首采对照，非成品」、手机端结果列表也会自动过滤；对这类旧记录仍可用结果卡里的「首采 / 二采对照」弹窗看两阶段图片、参数差异与按规则给出的「构图保持：高 / 中 / 低」（明确标注为规则判断，不是视觉分析）。
- 收藏 / 入选标记：作品列表可按「全部 / ★ 已入选 / 未入选」筛选，卡片右上角 ★ 一键标记，详情面板可设 0–5 星评分与备注。这些字段只记录“这是当前最佳版本”，**不改变任何生成参数、也不会新建版本**，桌面与手机共用同一份数据（`POST /api/rpg/library/favorite`）。
...
- **离线翻译（Argos Translate）**：不用 API Key、不联网。入口：**每个提示词框自己的「翻译」按钮**（与「清空 / 粘贴」同排，共 12 个）+ 中文描述转换卡里的「🧩 离线翻译（Argos）」。
  - **方向按框里内容自动选**：框里有中文 → 中文→英文，本地 101 条词表优先（蓝发 → `blue hair`），剩下的才交 Argos；英文标签与分隔符原样保留，翻完实时回写该框，**再点一次「翻译」撤销**（状态位就写在该框按钮后面 `data-translate-status`，不跑到别的区域）。
  - 框里是**纯英文** → 直接给**英文→中文对照**：不会把中文写进框里（英文才是最终提示词），而是在该分区弹出「解析词条」列表，一行一个 `english = 中文`（如 `holding pillow = 拿着枕头`、`looking at viewer = 看向观众`），**点整行就能删掉这个词、再点恢复**，也可以点 ↻ 单条重译；删完实时写回该框。适合拿来一段别人写的英文提示词，逐条看懂再删改。
  - 撤销只在“内容还是上次翻译结果”时生效：清空 / 粘贴 / 手动改框 / 换预设 / 恢复快照后，该框的上一次翻译快照立即作废，再点「翻译」是处理当前内容，不会把你改过的文本回退成旧的。
  - 段数不再受限制：前端按 32 段自动分批顺序请求再按原序合并（状态行显示「翻译中… 第 i/n 批」），逐框翻译、整段翻译与解析词条共用同一实现。
  - 原先重复的「🧩 分区中文→英文（离线）」按钮已移除：逐框「翻译」已经完全覆盖它，同一张卡上不必摆两个入口。
  - 需要一份本机 Argos 翻译器目录（含 `venv/` 与 `data/argos-translate/packages/translate-zh_en-*`、`translate-en_zh-*`）；默认按 `G:\edge download\webp_to_png_converter_v2\argos-translate`、`G:\ComfyUI\argos-translate` 探测，也可用环境变量 `EASY_PANEL_ARGOS_HOME` 指定。面板会把模型与缓存都放在该目录（XDG_* 指向它，不写 C 盘），调用时才起子进程，翻完即退。
- **🇨🇳 解析词条（英文→中文对照）**：把别人整段英文提示词拆成可理解、可删除、可恢复的词条，**英文始终是最终提示词**（中文只是解释）。做法：逗号/分号/换行切分（短语整体翻，不拆单词）→ **保护结构 token**（`<lora:...>`、`score_9`、`1girl`、`artist:name` 标 🔒、不翻译）→ EN→ZH 走离线批量翻译（自动分批 + 服务端/浏览器双层词条缓存 + 词表反查）→ 紧凑列表里点整行 = 保留/删除，**实时同步回原分区**；带「全保留 / 全删除 / 仅看已删除 / 重新解析 / 恢复原文 / 单条 ↻ 重译」。与提示词方言（prompt_dialect）严格隔离：这里只做理解与删除，不改写 canonical tag 写法。
...
- 提示词冲突医生：识别高权重、正负向对抗、发型/姿势/人数等语义冲突。
- 自动提示词来源追踪：模型质量词、LoRA 触发词、动态负面词和场景兜底均可见、可单独关闭。
- 严格 LoRA 对比实验室：冻结同一生成基线，只扫描 LoRA 顺序或单个权重。
- 本地生成快照：保存完整参数、最终提示词和模型文件指纹，可恢复、复现并检查环境差异。
...
8. 点击“编译并检查”，确认没有红色错误。
9. 尺寸先点比例「2:3」，再选推荐档位「832 × 1216」；要其它尺寸就拖长边滑块，或直接填长边 / 宽高。
10. 种子填 `-1`，生成数量填 `1`。
11. “高级参数”保持自动。
12. 点击“生成图片”。
...
如果最小测试成功，再尝试更大的画布（如手机竖图 768 × 1344、超宽横图 1536 × 768），然后逐项添加 LoRA 或其他功能。
...
最终提示词下方会检查加权标签、正负向重复和常见语义冲突。例如正向写
`(high ponytail:1.4)`、负向同时写 `hair down, loose hair` 时，会提示发型注意力
拉扯和权重过高。检查只给出原因，不会擅自删除或改写用户内容。

“自动提示词来源与开关”会分别显示模型质量词、LoRA 自动触发词、模型基础负面词、
安全等级负面词、动态结构保护词、空场景兜底和局部修复保护词。关闭某一项后重新编译，
该来源不会再加入最终文本；用户输入的正负向内容始终保留。
5. 固定原种子后生成，比较复现效果。
...
先设置固定 seed，再打开“严格 LoRA 对比实验室”：

1. “全部 LoRA 顺序排列”会冻结当前完整参数，建立顺序排列任务；2–4 个 LoRA 可用。
2. “单个 LoRA 权重扫描”只改变选中 LoRA 的权重，默认扫描 `0.50、0.65、0.80、0.95`。
3. 每个变体可使用 1–4 个连续 seed，用于判断结果是否只是单个 seed 的偶然表现。
4. 创建后的任务进入普通暂存队列，确认数量后再点击“发送队列”。

实验任务会把当时编译好的最终正负向冻结为手动原样提交；之后修改分区、模型或增强参数，
不会改变已经建立的实验任务。同一实验中的非目标参数保持一致。
...
- 放大倍率：`1.30×`
- 重绘幅度：`0.20–0.30`（默认 `0.25`）优先保留构图；`0.30–0.40` 强化细节但可能改动构图；超过 `0.40` 更接近重新生成
- 二次步数：`16`
- 二次 CFG：`4.0`

二采提示词：

- 模式：`继承首采`（二采直接用首采正负向）、`追加补充`（首采 + 补充词，推荐且为默认）、`完全独立`（只用二采两框，留空的一项沿用首采）。
- 「追加补充」只写局部细节、材质、表情、服装破损、发丝、皮肤细节；不要重复构图、镜头、人物位置。
- 快捷模板：`细节强化 / 发丝强化 / 服装强化 / 表情强化 / 光影强化`，点击只追加到二采补充框，不覆盖已有内容。
- 「优先保持首采构图」（默认开启）把二采重绘幅度上限锁到 `0.35`，**前端与后端都会限制，API / 手机调用同样生效**；补充词里出现 `close-up`、`full body`、`portrait` 等镜头/构图词时会给出提醒。
- 多人区域提示词开启时，二采提示词按“继承首采”处理，避免丢掉角色蒙版条件。
...
面板内置「机位控制」：滑杆值被折算成加权相机词（`from left` / `eye-level` / `close-up` …），与 BSK 插件的 `BSK_相机控制` 节点同一套参数语义，**不需要安装该插件**。

- 需要同时挂载配套 LoRA：`相机机位控制（BSK）`（在 `Anima_Soft_Illustration\03_通用功能`，作者建议权重 `0.6–1.0`）。面板里的「挂载机位 LoRA」按钮会按当前权重把它加入 LoRA 列表。
- 滑杆：左右 `X`（±1 = 背面）、上下 `Y`（正值俯视/鸟瞰，负值仰视，0 = 平视）、前后 `Z`（正值靠近 = 特写）、翻滚 `Roll`（`|值| ≥ 0.15` 输出 `dutch angle`）。
- 快捷按钮：正面 / 左机位 / 右机位 / 背面，鸟瞰 / 俯视 / 平视 / 仰视 / 正下，特写 / 近景 / 中景 / 全身 / 远景。
- 生成时机位词并入正向提示词并自动去重；预检提示词里的「机位控制（BSK 相机）」来源可查看实际词条。
- 只对 Anima 模型显示；关闭开关后不写入任何机位词。
...
| Anima Base | 默认「Anima 标准」34 步 · CFG 4.8 · `er_sde / sgm_uniform` | 一采预设：Anima 标准 / Anima 简洁（`er_sde / simple`，Denoise 1.0）/ Anima 柔和 / Anima 变化（30–40 步、CFG 4.0–5.5） |
...
| 生成快照与模型指纹 | `generation_snapshots.json` | 本机最多保留最近 200 条，不提交 Git |

每次从面板提交生成后都会建立快照。快照列表支持：

- “恢复到面板”：载入原模型、LoRA 顺序/权重、seed、尺寸和最终提示词，但不立即生成。
- “完全复现”：把原始完整 payload 原样加入任务队列。
- “检查环境差异”：比较当前 checkpoint、LoRA 快速内容指纹和面板版本。

文件指纹使用大小与文件首尾内容计算，目的是快速发现同名文件被替换或缺失；它不是完整文件
SHA-256。快照不会保存浏览器登录状态、AI 服务密钥或 Google 翻译密钥。
