...
- **离线翻译（Argos Translate）**：不用 API Key、不联网的中→英。入口：**每个提示词框自己的「翻译」按钮**（与「清空 / 粘贴」同排，点一下只翻这个框里的中文片段：本地 101 条词表优先（蓝发 → `blue hair`），剩下的中文才交 Argos；英文标签与分隔符原样保留，翻完实时回写该框，**再点一次「翻译」撤销**；纯英文框会提示「没有中文片段」；状态就写在该框按钮后面的 `data-translate-status` 上，不跑到别的区域）；中文描述转换卡里的「🧩 离线翻译（Argos）」（整段描述 → 英文，写自然语言分区，与 Google 直译同一条应用流程）。原来的「🧩 分区中文→英文（离线）」按钮已移除：逐框「翻译」已经完全覆盖它，同一张卡上不必摆两个入口。
  - 撤销只在“内容还是上次翻译结果”时生效：清空 / 粘贴 / 手动改框 / 换预设 / 恢复快照后，该框的上一次翻译快照立即作废，再点「翻译」是翻当前内容，不会把你改过的文本回退成旧的。
...
- **🇨🇳 解析词条（Prompt Explainer）**：把别人整段英文提示词拆成可理解、可删除、可恢复的词条，**英文始终是最终提示词**（中文只是解释）。做法：逗号/分号/换行切分（短语整体翻，不拆单词）→ **保护结构 token**（`<lora:...>`、`score_9`、`1girl`、`artist:name` 标 🔒、不翻译）→ EN→ZH 走离线批量翻译（自动分批 + 服务端/浏览器双层词条缓存 + 词表反查）→ 紧凑列表里点整行 = 保留/删除，**实时同步回原分区**；带「全保留 / 全删除 / 仅看已删除 / 重新解析 / 恢复原文 / 单条 ↻ 重译」。与提示词方言（prompt_dialect）严格隔离：这里只做理解与删除，不改写 canonical tag 写法。
