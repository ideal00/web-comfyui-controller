...
import EasyPanelMobileApp from './components/EasyPanelMobileApp'
import GameDrawer, { type GameDrawerProps } from './components/GameDrawer'
import GameSettingsDialog from './components/GameSettingsDialog'
import GlobalSettingsDialog from './components/GlobalSettingsDialog'
import VisualSettingsDialog from './components/VisualSettingsDialog'
import VisualCgLayer from './components/VisualCgLayer'
...
import { useEasyPanelVisual } from './hooks/useEasyPanelVisual'
import { normalizeProviderModelId, streamCompletion } from './services/openai'
import type { CompletionModelFallbackInfo, CompletionRetryInfo, CompletionUsage } from './services/openai'
...
function RpgBoxApp() {
  ...
  const [retryStatus, setRetryStatus] = useState<CompletionRetryInfo | null>(null)
  const [modelFallbackStatus, setModelFallbackStatus] = useState<CompletionModelFallbackInfo | null>(null)
  const [cancelling, setCancelling] = useState(false)
  const [retryAvailable, setRetryAvailable] = useState(false)
  const [retryForcedInput, setRetryForcedInput] = useState<string | undefined>(undefined)
  ...
  const activeProvider = configuredProvider ? {
    ...configuredProvider,
    model: normalizeProviderModelId(activeGame.aiSettings.model || configuredProvider.model, configuredProvider.baseUrl),
  ...
  const visualController = useEasyPanelVisual({
    game: activeGame,
    assistant: latestAssistant,
    segments: latestParsed.segments,
    busy,
  })
  ...
  async function summarizeChapterMemory(
    ...
    try {
      const rawSummary = await streamCompletion({
        ...
        signal,
        messages,
        fallbackModels: game.aiSettings.enableFallbackModels ? game.aiSettings.fallbackModels : [],
  ...
  async function summarizeDistantMemory(existing: string, chapters: ChapterMemory[], signal: AbortSignal, additionalInstructions = '', onDebug?: (entry: MemorySummaryDebugEntry) => void) {
    ...
    try {
      const rawSummary = await streamCompletion({
        ...
        signal,
        messages,
        fallbackModels: activeGame.aiSettings.enableFallbackModels ? activeGame.aiSettings.fallbackModels : [],
        onUsage: (next) => { usage = next },
      })
      responses.push(rawSummary)
      onDebug?.(formatMemorySummaryDebug('远期记忆整理', debugRequest, responses, usage))
  ...
  async function continueTruncatedResponse() {
    ...
    try {
      const continuation = await streamCompletion({
        ...
        fallbackModels: gameSnapshot.aiSettings.enableFallbackModels ? gameSnapshot.aiSettings.fallbackModels : [],
        signal: controller.signal,
        onUsage: (usage) => { continuationUsage = usage },
        onRetry: (info) => setRetryStatus(info),
        onModelFallback: (info) => setModelFallbackStatus(info),
        onToken: (content) => {
  ...
  async function repairLatestResponseFormat() {
    ...
    try {
      ...
      const fullText = await streamCompletion({
        ...
        fallbackModels: originalGame.aiSettings.enableFallbackModels ? originalGame.aiSettings.fallbackModels : [],
        signal: controller.signal,
  ...
  async function sendTurn(forcedInput?: string) {
    ...
    const primaryModel = normalizeProviderModelId(activeProvider.model, activeProvider.baseUrl)
    const usedModels = [primaryModel]
    const storedUserMessage = { ...userMessage, requestContent, requestSegments, providerModel: primaryModel, providerModelsUsed: [...usedModels] }
    ...
    const restoreTurnState = () => {
      ...
      if (submittedManualDisplayContentMode) setManualDisplayContentModes((current) => ({ ...current, [gameId]: submittedManualDisplayContentMode }))
      if (sceneAnnouncementPendingMessageRef.current === pendingAssistant.id) sceneAnnouncementPendingMessageRef.current = null
      if (narrativeModeFlashPendingMessageRef.current === pendingAssistant.id) narrativeModeFlashPendingMessageRef.current = null
    }
    ...
    try {
      ...
      const fullText = await streamCompletion({
        ...
        onModelFallback: (info) => {
          if (!usedModels.includes(info.toModel)) usedModels.push(info.toModel)
          setModelFallbackStatus(info)
        },
        onToken: (content) => {
          const debugUserMessage = { ...storedUserMessage, providerModel: usedModels.at(-1) ?? primaryModel, providerModelsUsed: [...usedModels] }
          updateGame(gameId, (game) => ({
            ...game,
            messages: [...gameSnapshot.messages, debugUserMessage, { ...pendingAssistant, content: standardResponse(content, responseParseContext), rawContent: content }],
            updatedAt: Date.now(),
          }))
        },
      })
      ...
      const completedUser = { ...storedUserMessage, providerModel: usedModels.at(-1) ?? primaryModel, providerModelsUsed: [...usedModels], chapterTitle: turnChapter }
      ...
      restoreTurnState()
      if (controller.signal.aborted) {
        setError('本次请求已取消，原输入和剧情状态已恢复。')
      } else {
        setError(toErrorMessage(requestError))
        setRetryForcedInput(forcedInput)
    ...
    <div className="app-shell">
      <header className="topbar">
        ...
        <div className="topbar-actions">
          ...
          <button className={`topbar-action-button ${visualController.status === 'error' ? 'visual-error' : ''}`} onClick={() => setVisualSettingsOpen(true)} title="Easy Panel 剧情 CG"><Server size={18} /><span>CG</span></button>
      ...
      <main className="rpg-stage-shell">
        ...
        >
          <VisualCgLayer
            source={visualController.cgSource}
            status={visualController.status}
            error={visualController.error}
            dimPercent={visualController.settings.dimPercent}
            enabled={visualController.settings.enabled}
            onGenerate={() => void visualController.generateNow()}
            onClear={() => void visualController.clearCg()}
          />
          ...
          {(!hasUsableProvider || error || protocolAlertKey) && <div className="stage-alerts" aria-live="polite">
            ...
            {error && <div className="stage-alert error"><span>{error}</span>{retryAvailable && <button type="button" className="stage-alert-retry" onClick={() => { const forcedInput = retryForcedInput; setRetryAvailable(false); void sendTurn(forcedInput) }}>重试</button>}<button type="button" onClick={(event) => { event.stopPropagation(); setError('') }} title="关闭错误提示"><X size={15} /></button></div>}
              ...
              <div className="playback-info">
                <div className="narrative-position">{displayChapterTitle || '章节间过渡'}-{displayChapterTurnCount}</div>
                  <div className="generation-status" aria-live="polite">
                   <span>{streamingParsed.segments.length ? `${segmentIndex + 1} / ${streamingParsed.segments.length}` : '0 / 0'} · <span className="generation-label">{cancelling ? '取消中' : modelFallbackStatus ? `主模型繁忙，正在尝试备用模型 ${modelFallbackStatus.toModel}${retryStatus ? ` · ${retryStatus.message}（第 ${retryStatus.attempt}/${retryStatus.maxAttempts} 次重试）` : ''}` : retryStatus ? `${retryStatus.message}（第 ${retryStatus.attempt}/${retryStatus.maxAttempts} 次重试）` : continuingResponse ? '补全中' : '生成中'}</span></span>
                  <button className="send-button stop" onClick={() => { setCancelling(true); abortRef.current?.abort() }} disabled={cancelling} title={cancelling ? '正在取消请求' : '取消请求'}><CircleStop size={20} /></button>
                </div>
              </div>
      ...
      {visualSettingsOpen && <VisualSettingsDialog value={visualController.settings} onChange={visualController.setSettings} onClose={() => setVisualSettingsOpen(false)} />}
...
function RawResponseDialog({ requestSegments, content, repairContent, memorySummaryEntries, inputTokens, outputTokens, modelsUsed, onClose }: { requestSegments: DebugPromptSegment[]; content: string; repairContent?: string; memorySummaryEntries: MemorySummaryDebugEntry[]; inputTokens?: number; outputTokens?: number; modelsUsed: string[]; onClose: () => void }) {
    ...
    <div className="modal-layer" role="dialog" aria-modal="true" aria-label="对话 Debug">
      ...
      <section className="modal debug-modal">
        ...
        <div className="debug-tab-content">
           ...
           {activeTab === 'input' && modelsUsed.length > 0 && <div className="debug-model-usage"><span>本轮实际模型</span><strong>{modelsUsed.join(' → ')}</strong></div>}
...
function App() {
  const mobileControllerRequested = typeof window !== 'undefined'
    && new URLSearchParams(window.location.search).get('easy-panel') === '1'
  if (isAndroidRuntime() || mobileControllerRequested) return <EasyPanelMobileApp />
  return <RpgBoxApp />
}
