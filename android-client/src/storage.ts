...
import { normalizeProviderModelId, normalizeProviderModelIds } from './services/openai'
import type { ChatMessage, GameAiSettings, GameSession, GameState, NarrativeProgress, ProviderProfile } from './types'
...
function normalizeProviderProfile(provider: ProviderProfile): ProviderProfile {
  ...
  const model = normalizeProviderModelId(provider.model ?? '', baseUrl)
  const sourceModels = provider.models?.length ? provider.models : model ? [model] : []
  return {
    ...provider,
    model,
    models: normalizeProviderModelIds(sourceModels, baseUrl),
  }
}

function normalizeGameAiSettings(aiSettings: GameAiSettings, providers: ProviderProfile[]): GameAiSettings {
  const provider = providers.find((item) => item.id === aiSettings.providerId)
  return {
    ...aiSettings,
    model: normalizeProviderModelId(aiSettings.model ?? '', provider?.baseUrl ?? ''),
    enableFallbackModels: aiSettings.enableFallbackModels ?? false,
    fallbackModels: normalizeProviderModelIds(
      Array.isArray(aiSettings.fallbackModels) ? aiSettings.fallbackModels : [],
      provider?.baseUrl ?? '',
    ),
  }
}
  ...
  try {
    ...
    if (parsed.providers) {
      parsed.providers = parsed.providers.map((provider) => ({
        ...normalizeProviderProfile(provider),
    ...
    parsed.games = (parsed.games ?? []).map((game) => {
      ...
      const aiSettings = { ...createDefaultAiSettings(fallbackProvider), ...legacy.aiSettings }
      ...
      aiSettings: normalizeGameAiSettings(aiSettings, parsed.providers ?? []),
...
export async function saveState(state: PersistedState): Promise<void> {
  const providers = state.providers.map(normalizeProviderProfile)
  const games = state.games.map((game) => ({
    ...game,
    aiSettings: normalizeGameAiSettings(game.aiSettings, providers),
  }))
  await writeStoredState(KEY, JSON.stringify({ ...state, providers, games }))
}
