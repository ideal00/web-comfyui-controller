...
describe('latestDebugExchange', () => {
  ...
  it('exposes only the model IDs used across a busy-model fallback chain', () => {
    const messages: ChatMessage[] = [
      { id: 'u1', role: 'user', content: 'A', providerModel: 'gemini-3.5-flash-lite', providerModelsUsed: ['gemini-3.5-flash-lite', 'gemini-3.1-flash-lite'], createdAt: 1 },
      { id: 'a1', role: 'assistant', content: '回复', createdAt: 2 },
    ]
    expect(latestDebugExchange(messages).modelsUsed).toEqual(['gemini-3.5-flash-lite', 'gemini-3.1-flash-lite'])
  })
})
