import { ArrowDown, ArrowUp, BookOpen, Compass, Download, Eye, FileUp, ImagePlus, Plus, SlidersHorizontal, Star, Trash2, Users, X } from 'lucide-react'
...
import { isGeminiOpenAiBaseUrl, isGeminiSamplingRestrictedModel, normalizeProviderModelId, normalizeProviderModelIds, partitionGeminiModels } from '../services/openai'
...
export default function GameSettingsDialog({ game, games, providers, fullSystemPrompt, onClose, onChange }: Props) {
  ...
  const selectedProviderIsGemini = isGeminiOpenAiBaseUrl(selectedProvider?.baseUrl ?? '')
  const selectedModel = normalizeProviderModelId(game.aiSettings.model, selectedProvider?.baseUrl ?? '')
  const selectedModelSamplingRestricted = isGeminiSamplingRestrictedModel(selectedModel, selectedProvider?.baseUrl ?? '')
  ...
  const modelGroups = useMemo(() => {
    const normalized = Array.from(new Set([
      ...normalizeProviderModelIds(
        [...(selectedProvider?.models?.length ? selectedProvider.models : selectedProvider?.model ? [selectedProvider.model] : []), game.aiSettings.model].filter(Boolean),
        selectedProvider?.baseUrl ?? '',
      ),
    ].filter(Boolean)))
    if (!selectedProviderIsGemini) return { textModels: normalized, specializedModels: [] }
    return partitionGeminiModels(normalized)
  }, [game.aiSettings.model, selectedProvider, selectedProviderIsGemini])
  const models = [...modelGroups.textModels, ...modelGroups.specializedModels]
  const configuredFallbackModels = normalizeProviderModelIds(game.aiSettings.fallbackModels ?? [], selectedProvider?.baseUrl ?? '')
    .filter((model) => model !== selectedModel)
  const fallbackCandidates = models.filter((model) => model !== selectedModel && !configuredFallbackModels.includes(model))
  ...
  function patchGame(patch: Partial<GameSession>) {
    const nextGame = { ...game, ...patch, updatedAt: Date.now() }
    const provider = providers.find((item) => item.id === nextGame.aiSettings.providerId)
    onChange({
      ...nextGame,
      aiSettings: {
        ...nextGame.aiSettings,
        model: normalizeProviderModelId(nextGame.aiSettings.model, provider?.baseUrl ?? ''),
        fallbackModels: normalizeProviderModelIds(nextGame.aiSettings.fallbackModels ?? [], provider?.baseUrl ?? '')
          .filter((model) => model !== normalizeProviderModelId(nextGame.aiSettings.model, provider?.baseUrl ?? '')),
        enableFallbackModels: nextGame.aiSettings.enableFallbackModels ?? false,
      },
    })
  }
  ...
  function commitNumericSettings(closeAfter = false) {
    ...
    if (contextTurns !== game.aiSettings.contextTurns
      || recentChapterLimit !== game.memory.recentChapterLimit
      || maxTokens !== game.aiSettings.maxTokens
      || normalizedGame.aiSettings.model !== game.aiSettings.model
      || JSON.stringify(normalizedGame.aiSettings.fallbackModels ?? []) !== JSON.stringify(game.aiSettings.fallbackModels ?? [])
      || normalizedGame.aiSettings.enableFallbackModels !== game.aiSettings.enableFallbackModels) {
      onChange(normalizedGame)
    }
  ...
  function patchFallbackModels(fallbackModels: string[], enableFallbackModels = game.aiSettings.enableFallbackModels ?? false) {
    patchGame({
      aiSettings: {
        ...game.aiSettings,
        enableFallbackModels,
        fallbackModels: normalizeProviderModelIds(fallbackModels, selectedProvider?.baseUrl ?? '')
          .filter((model) => model !== selectedModel),
      },
    })
  }
  ...
  function addFallbackModel(model: string) {
    const normalized = normalizeProviderModelId(model, selectedProvider?.baseUrl ?? '')
    if (!normalized || normalized === selectedModel || configuredFallbackModels.includes(normalized)) return
    patchFallbackModels([...configuredFallbackModels, normalized])
  }

  function removeFallbackModel(model: string) {
    patchFallbackModels(configuredFallbackModels.filter((item) => item !== model))
  }

  function moveFallbackModel(index: number, offset: -1 | 1) {
    const targetIndex = index + offset
    if (targetIndex < 0 || targetIndex >= configuredFallbackModels.length) return
    const next = [...configuredFallbackModels]
    const [moved] = next.splice(index, 1)
    next.splice(targetIndex, 0, moved)
    patchFallbackModels(next)
  }
        ...
        <div className="game-settings-content">
          {tab === 'ai' && <section className="game-tab-panel ai-settings-panel">
            <div className="form-section"><h3>接口与模型</h3>
              <label>API 配置<select value={selectedProvider?.id ?? ''} onChange={(event) => { const provider = providers.find((item) => item.id === event.target.value); if (provider) patchGame({ aiSettings: { ...game.aiSettings, providerId: provider.id, model: normalizeProviderModelId(provider.models[0] ?? provider.model, provider.baseUrl), enableFallbackModels: false, fallbackModels: [] } }) }}>{providers.map((provider) => <option value={provider.id} key={provider.id}>{provider.name}</option>)}</select></label>
              <label>模型<select value={selectedModel} onChange={(event) => patchGame({ aiSettings: { ...game.aiSettings, model: normalizeProviderModelId(event.target.value, selectedProvider?.baseUrl ?? '') } })}>{selectedProviderIsGemini ? <><optgroup label="推荐文本模型">{modelGroups.textModels.map((model) => <option value={model} key={model}>{model}</option>)}</optgroup>{modelGroups.specializedModels.length > 0 && <optgroup label="其他模型（不推荐用于纯文本 RPG）">{modelGroups.specializedModels.map((model) => <option value={model} key={model}>{model}</option>)}</optgroup>}</> : models.map((model) => <option value={model} key={model}>{model}</option>)}</select></label>
            </div>
            <div className="form-section fallback-model-section"><div className="form-section-head"><div><h3>繁忙时备用模型</h3><span className="section-meta">默认关闭 · 按顺序尝试</span></div></div><label className="setting-toggle"><input type="checkbox" checked={Boolean(game.aiSettings.enableFallbackModels)} disabled={!game.aiSettings.enableFallbackModels && !configuredFallbackModels.length && !fallbackCandidates.length} onChange={(event) => patchFallbackModels(configuredFallbackModels, event.target.checked)} /><span><strong>启用备用模型</strong><small>主模型在 429、503 或临时 5xx 重试耗尽后，才会尝试下方模型；400、401、403、404 不会切换。</small></span></label>{selectedProviderIsGemini && <p className="field-note">仅针对当前 RPG 的文本对话生效，不会自动修改主模型。请优先选择稳定的 Flash-Lite/Flash 模型。</p>}{configuredFallbackModels.length ? <div className="fallback-model-list">{configuredFallbackModels.map((model, index) => <div className="fallback-model-row" key={model}><span className="fallback-model-order">{index + 1}</span><span className="fallback-model-name">{model}</span><div className="fallback-model-actions"><button type="button" className="fallback-order-button" onClick={() => moveFallbackModel(index, -1)} disabled={index === 0} title="上移"><ArrowUp size={14} /></button><button type="button" className="fallback-order-button" onClick={() => moveFallbackModel(index, 1)} disabled={index === configuredFallbackModels.length - 1} title="下移"><ArrowDown size={14} /></button><button type="button" className="fallback-remove-button" onClick={() => removeFallbackModel(model)} title={`移除备用模型 ${model}`}><X size={14} /></button></div></div>)}</div> : <p className="empty-models">尚未设置备用模型。请先在全局设置中添加模型。</p>}<label>添加备用模型<select value="" disabled={!fallbackCandidates.length} onChange={(event) => addFallbackModel(event.target.value)}><option value="">{fallbackCandidates.length ? '选择模型…' : '没有可添加的模型'}</option>{fallbackCandidates.map((model) => <option value={model} key={model}>{model}</option>)}</select></label></div>
            <div className="form-section"><h3>模型参数</h3>
              ...
              <ParameterSlider label="温度" min={0} max={2} step={0.05} value={game.aiSettings.temperature} disabled={selectedModelSamplingRestricted} onChange={(temperature) => patchGame({ aiSettings: { ...game.aiSettings, temperature } })} />
              <ParameterSlider label="Top P" min={0} max={1} step={0.05} value={game.aiSettings.topP} disabled={selectedModelSamplingRestricted} onChange={(topP) => patchGame({ aiSettings: { ...game.aiSettings, topP } })} />
              <ParameterSlider label="存在惩罚" min={-2} max={2} step={0.05} value={game.aiSettings.presencePenalty} disabled={selectedProviderIsGemini} onChange={(presencePenalty) => patchGame({ aiSettings: { ...game.aiSettings, presencePenalty } })} />
              <ParameterSlider label="频率惩罚" min={-2} max={2} step={0.05} value={game.aiSettings.frequencyPenalty} disabled={selectedProviderIsGemini} onChange={(frequencyPenalty) => patchGame({ aiSettings: { ...game.aiSettings, frequencyPenalty } })} />
              {selectedProviderIsGemini && <p className="field-note">Gemini OpenAI 兼容接口当前不发送存在惩罚和频率惩罚；切换普通 OpenAI 兼容接口后才会生效。</p>}
              {selectedModelSamplingRestricted && <p className="field-note">Gemini 3.6/3.7 不支持已弃用的 temperature、top_p、top_k；本次请求不会发送这些字段。</p>}
