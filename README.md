...
- **离线翻译（Argos Translate）**：不用 API Key、不联网的中→英。两处入口：中文描述转换卡里的「🧩 离线翻译（Argos）」（整段描述 → 英文，写自然语言分区，与 Google 直译同一条应用流程）；结构化提示词编译器里的「🧩 分区中文→英文（离线）」（**本地 101 条词表优先**（蓝发 → `blue hair`，比通用机翻准），剩下的中文片段才交 Argos；原文里的英文标签与分隔符原样保留，翻完实时写回分区，并给一个「撤销本次翻译」按钮）。
