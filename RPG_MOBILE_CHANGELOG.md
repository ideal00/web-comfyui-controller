...
## mobile-v1.4.6（2026-09-14）

- 结果卡新增「执行链」：直接使用电脑端 `generation_plan()` 的实际计划（阶段、步数、局部重绘、计划输出），提交、轮询、恢复未完成任务时都可见；任务中心也显示队列任务的计划。
- 作品库新增产物阶段（首采 / 高清二采 / 细节重绘 / 面部修复 / 手部修复 / 脚部修复 / 输出放大）与「⏳ 文件保存中」：文件还在写盘时不会当成图片损坏，也不能下载或打开原图。
- 失败信息改用电脑端友好错误：界面只显示「标题：原因 / 建议」，`code` / `confidence` 等内部术语折叠在「技术详情」里；旧任务回退到一行错误文本。
- Anima 高清重建说明澄清：该功能内部已包含 Anime6B 超分，因此与「输出增强」互斥（避免重复放大）；只想纯放大时用输出增强。
...
- Easy Panel 的鉴权图片接口支持 `preview=1` 生成并缓存 360px 预览图；原图下载和查看仍使用原始尺寸。
- 原图预览改为应用内触摸手势：双指缩放、单指拖动、双击放大/还原，并提供“还原”按钮。
- Android versionCode 升级为 `1004004`，配套 Easy Panel 服务端 `2.2.2`；仅生成本地 Debug APK，未推送仓库或发布 GitHub Release。
