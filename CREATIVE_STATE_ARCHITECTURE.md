...
- “复现到当前表单”“换 Seed 到当前表单”和“继续编辑”只更新当前编辑状态；不会自动选择某个输出作为图生图 / 局部重绘 / 修复输入。下一次用户显式点击底部“生成图片”时才附加一次父作品关系，服务端接受后清除，失败保留供重试，也可在表单中取消关联。
...
- “复现到当前表单”“换 Seed 到当前表单”“继续编辑”只调用已有表单恢复能力，不调用 `/api/generate`、`/api/generate-batch` 或 `/api/rpg/generate`；页面会明确提示用户确认后手动点击“生成图片”。恢复不会自动挑选具体 artifact，也不会把继续编辑冒充为 `img2img`；复现 / 继续编辑保留已记录的 operation，换 Seed 固定使用 `seed_variant`。当前作品库尚不支持把用户选择的具体输出作为编辑输入并自动建立 `parentArtifactId`；只有未来真实接入该输入时才应建立它。
