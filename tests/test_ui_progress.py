...
class GenerationProgressUiTests(unittest.TestCase):
    def test_progress_bar_is_present_and_connected_to_comfy_progress_stream(self):
        ...
        result_position = html.index('id="result"', preview_start)
        settings_start = html.index('class="preview-settings"', preview_start)
        settings_body = html.index('class="preview-settings-body"', preview_start)
        self.assertGreater(progress_position, preview_start)
        # 图片区域顶到标题栏下方：进度条排在它前面（且只在生成时出现），宽高/队列/标签收进「⚙ 预览设置」。
        self.assertLess(progress_position, result_position)
        self.assertLess(progress_position, preview_end)
        self.assertLess(settings_start, result_position)
        self.assertLess(settings_body, result_position)
        for marker in ('id="previewWidth"', 'id="previewHeight"', 'id="queue"', 'id="samplerBadge"', "⚙ 预览设置"):
            self.assertIn(marker, html)
        self.assertNotIn('class="preview-controls"', html)
        ...
        self.assertIn("当前：${stage}", javascript)
        self.assertIn("function generationStageText(", javascript)
