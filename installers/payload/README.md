...
- 左侧「快捷工具」抽屉全部可折叠：输入框字号、我的功能、**透明背景角色 PNG**、中文描述转换、读取 AI 图片参数；折叠状态会记住（`localStorage easyPanelRailFoldV1`，默认全部折叠），点「读取 DeepSeek 回答 / 读图还原 / 手动修正透明图」会自动展开对应面板。透明背景角色 PNG 从生成设置页移到这里，切换创作标签页也不会消失。
