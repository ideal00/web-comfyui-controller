...
当前配套版本：Easy Panel 服务端 `2.2.2`，Android 客户端 `mobile-v1.4.6`（versionCode `1004006`）。协议仍为 RPG API v2。
...
开启高清二次采样时，Easy Panel 会把首采图另存为独立输出 `<prefix>_base_00001_.png`，供桌面面板做首采 / 二采对照；移动端结果接口会自动过滤这一类文件，只返回最终图（若一次任务只剩首采图则回退返回它）。
