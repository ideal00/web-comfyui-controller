...
// 把当前选区导成黑白蒙版（白=重绘区）：FLUX 智能修图/局部修复共用这一份导出。
window.handExportMask=function(){if(!hand.mask.width)return null;const out=document.createElement('canvas');out.width=hand.mask.width;out.height=hand.mask.height;const ctx=out.getContext('2d');ctx.fillStyle='#000';ctx.fillRect(0,0,out.width,out.height);ctx.drawImage(hand.mask,0,0);return{dataUrl:out.toDataURL('image/png'),image:(hand.file&&hand.file.name)||el('handSourceOutput')?.value||'',width:out.width,height:out.height}};
window.handSendToFlux=async function(){const result=window.handExportMask();if(!result){status('请先载入图片并圈出要修的区域。',true);return}if(typeof window.easyPanelFluxUseHandMask!=='function'){status('FLUX 修图模块未加载；请刷新面板。',true);return}el('handWorkbench').close();await window.easyPanelFluxUseHandMask(result);};
